import { stubConstructor, stubInterface, stubObject } from "./ts-sinon";
...
describe("ts-sinon", () => {
  describe("stubObject", () => {
    describe("when no methods or method map given", () => {
      it("returns stub es6 object with all methods stubbed", () => {
        ...
        const objectStub = stubObject<A>(A.name, object);
      ...
      it("returns stub literal object with all methods stubbed", () => {
        ...
        const objectStub = stubObject("object", object);
      ...
      it("allows to change stub values", () => {
        ...
        const object1Stub = stubObject("object1", object1);
        const object2Stub = stubObject("object2", object2);
    ...
    it('returns partial stub object with only "test" method stubbed when array with "test" has been given', () => {
      ...
      const objectStub = stubObject("object", object, ["test"]);
    ...
    it('returns partial stub object with "run" method stubbed and returning "some val" value when key value map { run: "some val" } has been given', () => {
      ...
      const objectStub = stubObject("object", object, { run: "some val" });
  ...
  describe("stubInterface", () => {
    ...
    it('returns stub object created from interface with all methods stubbed with "method2" predefined to return value of "abc" and "method1" which is testable with expect that has been called', () => {
      ...
      const interfaceStub: ITest = stubInterface<ITest>("ITest", {
      ...
      expect(object.run(expectedMethod2Arg)).to.equal(
        expectedMethod2ReturnValue
      );
    ...
    it("gives an access to method stubs of the stub object created from interface", () => {
      ...
      const interfaceStub = stubInterface<ITest>("ITest");
    ...
    it("stubs method to return resolved Promise with another interface stub", async () => {
      ...
      const interfaceTestStub = stubInterface<Test>("ITest");
      const interfaceITestStub = stubInterface<ITest>("ITest");
    ...
    describe("stub class while avoiding calling constructor", () => {
      ...
      it("returns stub object create from class without calling constructor", async () => {
        const stubWithoutCallingCtor = stubInterface<A>(
          "ITest",
          {},
          { getter: 10, readOnly: 5, asycFunc: cy.stub().resolves(8) }
        );
        expect(await stubWithoutCallingCtor.asycFunc()).to.equal(8);

        stubWithoutCallingCtor.asycFunc.returns(Promise.resolve(5));
        expect(await stubWithoutCallingCtor.asycFunc()).to.equal(5);
        ...
        expect(stubWithoutCallingCtor.propertyFunction).to.have.been.calledWith(
          11
        );
